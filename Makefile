...
gen: clean# spec/openapi.json
	docker run --rm -v `pwd`:/builder openapitools/openapi-generator-cli:${OPENAPI_GENERATOR_VERSION} generate --git-user-id ${GIT_USER_ID} --git-repo-id ${GIT_REPO_ID} -i /builder/spec/openapi.json -g python -o /builder --package-name koyeb --additional-properties enumClassPrefix=true --additional-properties isGoSubmodule=true --additional-properties generateInterfaces=true

.PHONY: clean
clean:
	rm -rf koyeb test docs
